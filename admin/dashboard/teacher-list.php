...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update_student"])) {
    ...
    $update_sql = "UPDATE teachers SET first_name=?, last_name=?, middle_name=?, date_of_birth=?, gender=?, email=?, phone_number=?, address_line1=?, address_line2=?, city=?, state=?, zip_code=?, country=?, parent_guardian_name=?, parent_guardian_phone=?, parent_guardian_email=?, emergency_contact_name=?, emergency_contact_phone=? WHERE teacher_id=?";
    $update_stmt = mysqli_prepare($conn, $update_sql);
    mysqli_stmt_bind_param($update_stmt, "ssssssssssssssssssi", $first_name, $last_name, $middle_name, $date_of_birth, $gender, $email, $phone_number, $address_line1, $address_line2, $city, $state, $zip_code, $country, $parent_guardian_name, $parent_guardian_phone, $parent_guardian_email, $emergency_contact_name, $emergency_contact_phone, $teacher_id);
...
if (isset($_GET['admit_id'])) {
    ...
    $admit_sql = "UPDATE teachers SET status = 'enrolled' WHERE teacher_id = ?";
...
if($status == 'all' || $status == '') {
    $students_sql = "SELECT * FROM teachers";
    $students_result = $conn->query($students_sql);
} else {
    $students_sql = "SELECT * FROM teachers WHERE status = 'reserved'";
    $students_result = $conn->query($students_sql);
}
    ...
    <table id="myTable" class="display">
        ...
        <tbody>
            ...
            if ($students_result && $students_result->num_rows > 0) {
                ...
                echo "<tr><td colspan='21'>No teachers found.</td></tr>";
            }
            ?>
        </tbody>
    </table>
