...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update_student"])) {
    $registrar_id = sanitize_input($conn, $_POST["registrar_id"]);
    ...
    $update_sql = "UPDATE registrars SET first_name=?, last_name=?, middle_name=?, date_of_birth=?, gender=?, email=?, phone_number=?, address_line1=?, address_line2=?, city=?, state=?, zip_code=?, country=?, parent_guardian_name=?, parent_guardian_phone=?, parent_guardian_email=?, emergency_contact_name=?, emergency_contact_phone=? WHERE registrar_id=?";
    $update_stmt = mysqli_prepare($conn, $update_sql);
    mysqli_stmt_bind_param($update_stmt, "ssssssssssssssssssi", $first_name, $last_name, $middle_name, $date_of_birth, $gender, $email, $phone_number, $address_line1, $address_line2, $city, $state, $zip_code, $country, $parent_guardian_name, $parent_guardian_phone, $parent_guardian_email, $emergency_contact_name, $emergency_contact_phone, $registrar_id);
...
if (isset($_GET['admit_id'])) {
    ...
    $admit_sql = "UPDATE registrars SET status = 'enrolled' WHERE registrar_id = ?";
...
if($status == 'all' || $status == '') {
    $students_sql = "SELECT * FROM registrars";
    $students_result = $conn->query($students_sql);
} else {
    $students_sql = "SELECT * FROM registrars WHERE status = 'reserved'";
    $students_result = $conn->query($students_sql);
}
        ...
        <tbody>
            ...
            if ($students_result && $students_result->num_rows > 0) {
                while ($row = $students_result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['registrar_id']) . "</td>";
                ...
                echo "<tr><td colspan='21'>No registrars found.</td></tr>";
            }
            ?>
        </tbody>
    </table>
