...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $confirm_new_password = sanitize_input($conn, $_POST['confirm_new_password']); // New field
    ...
    if ($updating_credentials) {
        if (empty($current_password)) {
            header("Location: index.php?error=Current password is required to change username or password");
            $conn->close();
            exit();
        }
        ...
        if (!empty($new_password) && $new_password !== $confirm_new_password) {
            header("Location: index.php?error=New password and confirmation do not match");
            $conn->close();
            exit();
        }
    }

    // Fields that can be updated in admins table
    $fields = [
    ...
    // Build the SQL UPDATE query for admins table
    $set_clause = [];
    $params = [];
    $types = '';
    $updates_made = false;
    ...
    if (!empty($set_clause)) {
        $sql = "UPDATE admins SET " . implode(', ', $set_clause) . " WHERE user_id = ?";
        $params[] = $user_id;
