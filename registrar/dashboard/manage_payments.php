...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update_payment"])) {
    // Check if all required fields are set before processing
    if (isset($_POST["student_id"]) && isset($_POST["amount"]) && isset($_POST["payment_date"]) && 
        ...
        if (mysqli_stmt_num_rows($check_stmt) == 0) {
            ...
            if (empty($payment_id)) { // Insert new payment
...
// $payments_sql = "SELECT s.student_id, s.first_name, s.last_name, p.*
//                  FROM students s
...
<table id="myTable" class="display">
    ...
    <tbody>
        ...
        if ($payments_result && $payments_result->num_rows > 0) {
            // echo "<pre>"; var_dump($payments_result->fetch_assoc()); echo "</pre>"; exit;
            while ($row = $payments_result->fetch_assoc()) {
                ...
                echo "</td>";
                echo "<td><input type='date' name='payment_date' value='" . (isset($row['payment_date']) ? htmlspecialchars($row['payment_date']) : date('Y-m-d')) . "'></td>";
                echo "<td>";
                ...
                echo "</select>";
                echo "</td>";
                echo "<td><input type='text' name='transaction_id' value='" . (isset($row['transaction_id']) ? htmlspecialchars($row['transaction_id']) : '') . "'></td>";
                echo "<td><input type='text' name='receipt_number' value='" . (isset($row['receipt_number']) ? htmlspecialchars($row['receipt_number']) : '') . "'></td>";
                echo "<td><input type='text' name='description' value='" . (isset($row['description']) ? htmlspecialchars($row['description']) : '') . "'></td>";
                echo "<td>";
